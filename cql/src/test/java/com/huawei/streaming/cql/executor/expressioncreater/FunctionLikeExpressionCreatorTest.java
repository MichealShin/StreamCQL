 ...
 *
 */
public class FunctionLikeExpressionCreatorTest
{
    ...
    {
        String like = "select id like 'xx%' from s1";
        ...
        assertTrue(desc.toString().equals("s1.id like 'xx%'"));
        
        LikeExpression likeExp = (LikeExpression)new FunctionLikeExpressionCreator().createInstance(desc, systemConfig);
     ...
     */
    @Test
    public void testNotLikeCreateInstance()
        throws StreamingException
    {
        String like = "select id not like 'xx%' from s1";
        SemanticAnalyzer analyzer = SemanticAnalyzerFactory.createAnalyzer(parser.parse(like), schemas);
        SelectAnalyzeContext selectContext = (SelectAnalyzeContext)analyzer.analyze();
        SelectClauseAnalyzeContext parseContext = selectContext.getSelectClauseContext();
        
        ExpressionDescribe desc = parseContext.getExpdes().get(0);
        assertTrue(desc.toString().equals("s1.id not like 'xx%'"));
