 ...
 *
 */
public class FunctionBetweenExpressionCreatorTest
{
    ...
    {
        String like = "select id between 1 and 100  from s1";
        ...
        assertTrue(desc.toString().equals("s1.id between 1 and 100"));
        
        BetweenExpression likeExp =
            (BetweenExpression)new FunctionBetweenExpressionCreator().createInstance(desc, systemConfig);
        assertTrue(likeExp.getType() == Boolean.class);
    }
    ...
    public void testNotCreateInstance()
        throws StreamingException
    {
        String like = "select id not between 1 and 100 from s1";
        SemanticAnalyzer analyzer = SemanticAnalyzerFactory.createAnalyzer(parser.parse(like), schemas);
        SelectAnalyzeContext selectContext = (SelectAnalyzeContext)analyzer.analyze();
        SelectClauseAnalyzeContext parseContext = selectContext.getSelectClauseContext();
        
        ExpressionDescribe desc = parseContext.getExpdes().get(0);
        assertTrue(desc.toString().equals("s1.id not between 1 and 100"));
