...
public class BetweenExpressionTest
{
        ...
        throws Exception
    {
        ...
        String deSerFileName = "between.xml";
        ...
        assertTrue(app.getAppName().equals("between"));
        
        IRichOperator agginfo = null;
        for (IRichOperator finfo : app.getFunctionstreams())
        {
            if (finfo.getOperatorId().equals("filter"))
            {
                agginfo = finfo;
            }
        }
        assertTrue(agginfo.getParallelNumber() == 1);
        assertTrue(agginfo instanceof FilterFunctionOp);
    }
