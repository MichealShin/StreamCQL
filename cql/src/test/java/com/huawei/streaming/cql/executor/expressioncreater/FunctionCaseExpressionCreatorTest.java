 ...
 *
 */
public class FunctionCaseExpressionCreatorTest
{
    ...
    {
        String like = "select case id when '1' then 1 when '2' then 2 when '3' then 3 else 99 end from s1";
        ...
        assertTrue(desc.toString().equals("case s1.id when '1' then 1 when '2' then 2 when '3' then 3 else 99 end "));
        
        CaseSimpleExpression likeExp =
            (CaseSimpleExpression)new FunctionCaseExpressionCreator().createInstance(desc, systemConfig);
    ...
    public void testNotCreateInstance()
        throws StreamingException
    {
        String like = "select case id when '1' then 1 when '2' then 2 when '3' then 3 end from s1";
        SemanticAnalyzer analyzer = SemanticAnalyzerFactory.createAnalyzer(parser.parse(like), schemas);
        SelectAnalyzeContext selectContext = (SelectAnalyzeContext)analyzer.analyze();
        SelectClauseAnalyzeContext parseContext = selectContext.getSelectClauseContext();
        
        ExpressionDescribe desc = parseContext.getExpdes().get(0);
        assertTrue(desc.toString().equals("case s1.id when '1' then 1 when '2' then 2 when '3' then 3 end "));
