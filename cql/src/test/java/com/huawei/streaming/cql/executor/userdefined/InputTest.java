...
/**
 * 自定义输入算子测试，使用API中的InputStream接口
 *
 */
public class InputTest
{
    ...
    public void writerTest()
        throws Exception
    {
        String serOutFileName = "input.out.xml";
        String serResultFileName = "input.res.xml";
    ...
    {
        TreeMap<String, String> config = Maps.newTreeMap();
        config.put("operator.tcpserver.port", "7999");
        config.put("operator.tcpserver.fixedlength", "966");
        
        InputStreamOperator input = new InputStreamOperator("input", 1);
        input.setArgs(config);
        input.setDeserializerClassName(WebDeserializer.class.getName());
        input.setRecordReaderClassName(TCPWebInput.class.getName());
        return input;
    }
